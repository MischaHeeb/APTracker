...
{
    /// <summary>
    /// Helper class used to easily create a valid database configuration.
    /// </summary>
    public static class DatabaseConfigurationHelper
    {
        ...
        private static string GetRequiredConfig(IConfiguration config, string key)
        {
            return config[key]
                   ?? throw new InvalidOperationException(
                       $"The environment variable '{key}' is required but was not found in your configuration.");
        }
        ...
        private static void ValidateConfiguration(DatabaseOptions dbOptions)
        {
            ...
            if (errors.Count > 0)
            {
                throw new ArgumentException("There were errors during database configuration: " +
                                            string.Join(", ", errors));
            }
        }
    }
